...
const searchAll = async (req, res = response) => {
    try {
        ...
        const [users, totalUsersRows, sheets, totalSheetsRows  ] = await Promise.all([
            User.find({ firstName: regex }, 'username firstName lastName email role google')
                .skip( offset )
                .limit( limit ),
            User.find().count(),
            Sheet.find({ name: regex }, 'name author description year license genres')
                .skip( offset )
                .limit( limit ),
            Sheet.find().count(),
...
const searchUsers = async (req, res = response) => {
    try {
        ...
        switch(table) {
            case 'users':
                data = await User.find({ firstName: regex })
                    .skip( offset )
                    .limit( limit );
                break;
            case 'sheets':
                data = await Sheet.find({ name: regex })
                    .populate('user', 'username firstName lastName email img')
                    .skip( offset )
                    .limit( limit );
                break;
            case 'interactions':
